...
def rolldice(diceString, serverName, userName):
	try:
		...
		if dice > 100000 or sides > 10000:
			raise Exception("diceroll failed")
		...
		for diceArg in diceString:
			if argNum > 1:
				mathArg = diceString[argNum].translate(str.maketrans({"x": r"*", "^": r"**"}))
	...
	if target in userData:
		...
		for stat, roll in statrolls.items():
			if (roll == 1):
				...
				mod = 4
			elif ((roll == 19) or (roll == 20)):
				mod = 5
			elif ((roll == 21) or (roll == 22)):
				mod = 6
			elif ((roll == 23) or (roll == 24)):
				mod = 7
			elif ((roll == 25) or (roll == 26)):
				mod = 8
			elif ((roll == 27) or (roll == 28)):
				mod = 9
			elif ((roll == 29) or (roll == 30)):
				mod = 10
			else:
				mod = 11
				userCheck = False
...
def hitpoints(serverName, userName, target=False, hit=0, damage=0):
	hitCheck = False
...
def inspiration(serverName, userName, targets):
	userCheck = []
