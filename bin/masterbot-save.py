...
def rolldice(theString, serverName, userName):
	try:
		for i in theString:
			if 'd' in i:
				diceString = theString.split('d')
...
async def on_message(message):
	...
	try:
		...
		if message_content_lower.startswith('+roll'):
			try:
				msgString = message_content_lower.split()
				theString = msgString.pop(0)
				diceroll = rolldice(theString, message.server.name, message.author)
				if diceroll:
					results, mathString, result = diceroll
					buff = [str(results)[b:b+2000] for b in range(0, len(str(results)), 2000)]
					if len(buff) > 10:
